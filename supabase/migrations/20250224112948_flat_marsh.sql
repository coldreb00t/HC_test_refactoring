...
-- Create updated view for client profiles with full photo URLs
      ...
      json_agg(
        json_build_object(
          ...
          'photos', (
            SELECT COALESCE(
              array_agg(
                'https://' || current_setting('supabase_config.project_ref') || '.supabase.co/storage/v1/object/public/client-photos/' || name
              ),
              ARRAY[]::text[]
            )
            FROM storage.objects
            WHERE bucket_id = 'client-photos'
            AND name LIKE 'nutrition-photos/' || c.id::text || '/' || cn.date::text || '/%'
          )
